"""Tests for the AbuseIPDB.com client module."""
...
MODULE = "ip_info.apis.abuseipdbcom"
API_NAME = "abuseipdbcom"
API_DISPLAY_NAME = "AbuseIPDB.com"
API_KEY = "test-abuseipdb-key"  # pragma: allowlist secret
...
def test_happy_path_all_flags(
    ...
    assert len(calls) == 1
    assert calls[0]["url"] == "https://api.abuseipdb.com/api/v2/check"
    assert calls[0]["headers"] == {"Accept": "application/json", "Key": API_KEY}
    assert calls[0]["params"] == {"ipAddress": "1.2.3.4", "maxAgeInDays": "365"}
    ...
    assert row["flags"] == "reports:5, risk:80, tor"
...
def test_recent_entry_skipped_then_next_ip_queried(
    ...
    assert len(calls) == 1
    assert calls[0]["params"]["ipAddress"] == "8.8.8.8"
    ip_rows = _rows(db_conn, IP_TABLE_NAME)
    assert len(ip_rows) == 1

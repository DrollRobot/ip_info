"""Client and parser for the ipregistry.co IP-info API."""
...
def ipregistryco(
    *,
    ...
    """Query ipregistry.co in bulk for the IP addresses and store parsed results in the database."""
    max_chunk_size = 1024
    params = {"key": api_key}
    ...
    for i in range(0, len(ips_to_query), max_chunk_size):
        ...
        if isinstance(results, dict):
            # select the nested "results" element if present, else wrap the dict in a list

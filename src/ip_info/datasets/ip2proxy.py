"""Import IP2Proxy LITE CSV datasets into the local IP-info database."""
...
def import_ip2proxy(file_path: str, chunk_size: int = 5000) -> None:
    """Parse an IP2Proxy LITE CSV file and insert one row per IP into the database in batches."""
    api_name = "ip2proxy"
    ...
    if file_name not in allowed_file_names:
        raise ValueError(
            f"Invalid file name '{file_name}'. Must be one of: "
            f"{', '.join(sorted(allowed_file_names))}"
        )
...
def cli() -> None:
    """Parse command-line arguments and run the IP2Proxy CSV import."""
    parser = argparse.ArgumentParser(description="Import IP2Proxy-LITE-PX12 CSV into the database")
    parser.add_argument(
        "file_path_pos",
        nargs="*",
